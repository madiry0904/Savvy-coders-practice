...
function checkTaskCompletionStatus(dataSet) {
  return dataSet.map(({ completed }) => ({ completed }));
}

function returnIncomplete(dataSet) {
  return dataSet.filter(({ completed }) => completed === false);
}

function returnCompleted(dataSet) {
  return dataSet.filter(({ completed }) => completed === true);
}
function returnCompletedIds(dataSet) {
  return dataSet
    .filter(({ completed }) => completed === true)
    .map(({ id }) => ({ id }));
}

function returnIdForCompletedStatus(dataSet) {
  return dataSet
    .filter(({ completed }) => completed === true)
    .map(({ id }) => ({ id }));
}

function getTitleForIncompleteStatus(dataSet) {
  return dataSet
    .filter(({ completed }) => completed === false)
    .map(({ title }) => ({ title }));
}

function delegateIncompleteTasksToUser(dataSet, workhorse) {
  return dataSet.filter(d => {
    if (d.completed === false) {
      return (d.userId = workhorse);
    }
  });
}
function getTitleForIncompleted(dataSet) {
  return dataSet
    .filter(({ completed }) => completed === false)
    .map(({ title }) => ({ title }));
}
console.log(getTitleForIncompleteStatus(todos));

console.log(checkTaskCompletionStatus(todos));
console.log(returnCompleted(todos));
console.log(returnCompletedIds(todos));
console.log(returnIncomplete(todos));
console.log(getUserIdForCompletedStatus(todos));
console.log(getTitleForIncompleteStatus(todos));
